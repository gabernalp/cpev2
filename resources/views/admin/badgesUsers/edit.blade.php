...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.badgesUser.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.badges-users.update", [$badgesUser->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $badgesUser->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('badge') ? 'is-invalid' : '' }}" name="badge_id" id="badge_id" required>
                    @foreach($badges as $id => $badge)
                        <option value="{{ $id }}" {{ (old('badge_id') ? old('badge_id') : $badgesUser->badge->id ?? '') == $id ? 'selected' : '' }}>{{ $badge }}</option>
                    @endforeach
                </select>
                @if($errors->has('badge'))
                    <div class="invalid-feedback">
