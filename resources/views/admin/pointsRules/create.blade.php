...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.points-rules.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="points_item">{{ trans('cruds.pointsRule.fields.points_item') }}</label>
                <input class="form-control {{ $errors->has('points_item') ? 'is-invalid' : '' }}" type="text" name="points_item" id="points_item" value="{{ old('points_item', '') }}" required>
